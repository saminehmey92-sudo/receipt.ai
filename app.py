...
import pillow_avif  # دعم صور AVIF
from pillow_heif import register_heif_opener # دعم صور آيفون HEIC

# تفعيل دعم صيغة HEIC عالمياً في التطبيق
register_heif_opener()

# --- 1. التصميم الاحترافي (Mobile-First UI) ---
...
st.markdown('<div class="main-header"><h1>📲 Return Manager Pro</h1><p>دعم شامل لجميع الصيغ والمميزات الاحترافية</p></div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.info("يتم حساب تاريخ انتهاء الاسترجاع تلقائياً بناءً على تاريخ اليوم.")

# --- 3. منطقة الرفع والمعالجة الشاملة (Universal Image Loader) ---
col_u, col_p = st.columns([1, 1])

with col_u:
    st.markdown("### 📸 خطوة 1: رفع الإيصال")
    # القائمة الكاملة للصيغ المدعومة بما فيها AVIF و HEIC
    uploaded_file = st.file_uploader("", type=['jpg', 'jpeg', 'png', 'webp', 'avif', 'heic'])

    if uploaded_file and api_key:
        try:
            # معالجة الصورة برمجياً لضمان توافقها مع OpenAI Vision
            ...
            if st.button("🔍 تحليل البيانات واستخراج المتجر"):
                with st.spinner('جاري قراءة البيانات بذكاء...'):
                    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
                    payload = {
                        "model": "gpt-4o",
                        "messages": [{"role": "user", "content": [
                            {"type": "text", "text": "Identify Store Name from the header. List items and prices. Format: STORE: [Name] ITEMS: Name | Price"},
                    ...
                    if 'choices' in res_json:
                        ...
                        if items:
                            ...
                            st.error("لم نتمكن من قراءة المنتجات. تأكد من وضوح الصورة.")
                    else:
                        st.error("فشل التحليل. تأكد من الـ API Key الخاص بك.")
        except Exception as e:
            st.error(f"خطأ في معالجة الملف: {str(e)}")

with col_p:
    if uploaded_file:
        st.image(uploaded_file, use_column_width=True, caption="معاينة الإيصال المرفوع")

# --- 4. واجهة المراجعة والتعديل الفوري ---
if "temp_items" in st.session_state and st.session_state.temp_items:
    ...
    if st.button("📥 حفظ في القائمة النهائية للمرتجعات"):
...
# --- 5. القائمة النهائية (الترتيب، الحذف، العرض) ---
if "refund_list" in st.session_state and len(st.session_state.refund_list) > 0:
    st.divider()
    st.header("🛒 المرتجعات النشطة")
    
    # فلتر الترتيب (تم توحيد اسم المتغير لضمان عدم حدوث خطأ)
    sort_by = st.radio("ترتيب حسب:", ["الأحدث", "الوقت المتبقي", "المتجر"], horizontal=True)
    ...
    for i, item in enumerate(st.session_state.refund_list):
        ...
        # نظام ألوان ذكي (أحمر إذا قارب التاريخ على الانتهاء)
        ...
        with col_card:
            st.markdown(f"""
                <div class="final-card" style="border-right-color: {status_color}">
                    <div>
                        <div style="font-size: 1.1rem; font-weight: bold; color: #333;">{item['name']}</div>
            ...
            """, unsafe_allow_html=True)
        with col_del:
            if st.button("❌", key=f"del_{i}", help="إزالة العنصر"):
    ...
    st.markdown(f"""
        <div style="background: #075E54; color: white; padding: 20px; border-radius: 15px; text-align: center; margin-top: 20px;">
            <span style="font-size: 1.1rem; opacity: 0.8;">إجمالي قيمة المرتجعات</span><br>
