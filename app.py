...
# تفعيل دعم صيغة HEIC عالمياً
register_heif_opener()

# --- 1. التصميم الاحترافي المستمر ---
...
st.markdown('<div class="main-header"><h1>📲 Return Manager Pro</h1><p>نظام الاستخراج الذكي المطور</p></div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    debug_mode = st.checkbox("إظهار الرد الخام (للتصحيح)")

# --- 3. منطقة المعالجة الشاملة ---
col_u, col_p = st.columns([1, 1])

with col_u:
    st.markdown("### 📸 ارفع الإيصال")
    ...
    if uploaded_file and api_key:
        try:
            ...
            image.save(img_byte_arr, format='JPEG', quality=95) # رفع الجودة لضمان وضوح النص
            img_b64 = base64.b64encode(img_byte_arr.getvalue()).decode('utf-8')
            
            if st.button("🔍 تحليل البيانات واستخراج المتجر"):
                with st.spinner('جاري تحليل الإيصال بدقة عالية...'):
                    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
                    payload = {
                        "model": "gpt-4o",
                        "messages": [{"role": "user", "content": [
                            {"type": "text", "text": "Analyze this receipt. Extract Store Name. For items, list every product with its final price. Format your response exactly as: \nSTORE: [Name]\nITEMS:\n[Item Name] | [Price]"},
                    ...
                    if 'choices' in res_json:
                        ...
                        if debug_mode: st.code(raw_output) # يظهر لك ماذا رأى الـ AI
                        
                        # استخراج المتجر
                        store_match = re.search(r"STORE:\s*(.*)", raw_output)
                        detected_store = store_match.group(1).strip() if store_match else "متجر غير معروف"
                        
                        # استخراج المنتجات (نمط بحث أكثر مرونة)
                        items = []
                        # نبحث عن السطر الذي يحتوي على اسم وسعر مفصولين بـ | أو : أو فراغ كبير
                        lines = raw_output.split('\n')
                        for line in lines:
                            # البحث عن اسم متبوع برقم عشري (سعر)
                            m = re.search(r"(.+?)\s*[|:]?\s*(\d+\.\d{2})", line)
                            if m and "STORE" not in line.upper() and "ITEMS" not in line.upper():
                                items.append({
                        ...
                        if items:
                            ...
                            st.error("لم نتمكن من قراءة المنتجات. جرب تفعيل 'وضع التصحيح' لرؤية المشكلة.")
                    else:
                        st.error("فشل التحليل: " + str(res_json.get('error', {}).get('message', 'خطأ غير معروف')))
        except Exception as e:
            st.error(f"خطأ في معالجة الملف: {str(e)}")

with col_p:
    if uploaded_file: st.image(uploaded_file, use_column_width=True, caption="معاينة الإيصال")
